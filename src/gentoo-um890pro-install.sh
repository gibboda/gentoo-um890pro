...
# Installation profile (Phase 1 profile selector).
# Accepted values (canonical names and short aliases):
#   core-openrc-dualkernel         (alias: core)     – base OS + dual-kernel only
...
EOF
    ...
    if ! chroot_run "eselect kernel set linux-${KERNEL_B_BASE_VERSION}-gentoo"; then
      echo "ERROR: Failed to select linux-${KERNEL_B_BASE_VERSION}-gentoo. Check eselect kernel list."
      exit 1
    fi
...
resolve_profile() {
  # Resolve INSTALL_PROFILE (canonical name or alias) to feature toggles.
  ...
  case "${p}" in
    ...
    full-ai)  p="full-openrc-dualkernel-kde-ai" ;;
  esac

  case "${p}" in
    core-openrc-dualkernel)
      INSTALL_KDE_PLASMA="no"
      INSTALL_ROCM="no"
      INSTALL_COMFYUI="no"
      INSTALL_BLENDER="no"
      INSTALL_DUAL_KERNEL="yes"
      ;;
    ...
    full-openrc-dualkernel-kde-ai)
      ...
      ;;
    *)
      echo "ERROR: unsupported INSTALL_PROFILE value: '${p}'" >&2
      echo "  Supported profiles: core-openrc-dualkernel (core)," >&2
      echo "                      desktop-openrc-dualkernel-kde (desktop)," >&2
      echo "                      full-openrc-dualkernel-kde-ai (full-ai)" >&2
      exit 1
      ;;
  esac

  # Persist the resolved canonical name for downstream reference.
  INSTALL_PROFILE="${p}"

  echo "============================================================"
  echo "Install profile : ${INSTALL_PROFILE}"
  echo "  INSTALL_DUAL_KERNEL : ${INSTALL_DUAL_KERNEL}"
  echo "  INSTALL_KDE_PLASMA  : ${INSTALL_KDE_PLASMA}"
  echo "  INSTALL_ROCM        : ${INSTALL_ROCM}"
  echo "  INSTALL_COMFYUI     : ${INSTALL_COMFYUI}"
  echo "  INSTALL_BLENDER     : ${INSTALL_BLENDER}"
  echo "============================================================"
}

...
- Partitions `OS_DISK` (ESP + Btrfs) and `DATA_DISK` (single partition for ZFS).
- Formats filesystems, creates Btrfs subvolumes, mounts under `/mnt/gentoo`.
- Downloads and extracts a Gentoo stage3 tarball, bind-mounts `/proc`, `/sys`, `/dev`, and runs install steps in a chroot.
- Configures base system settings (profile, locale/timezone, hostname), installs GRUB for EFI.
- Installs ZFS, creates pool + datasets, enables relevant services.
...
./scripts/bump-version.sh 2025.12.16 "Short summary of changes"
```
...
Optional `REPO_PAT`: if you add a PAT secret named `REPO_PAT`, the release workflow can prefer it for authenticated pushes/tags in maintainer runs.

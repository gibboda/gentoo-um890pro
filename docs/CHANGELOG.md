...
## [Unreleased]
- Fix singular “pre-enforcement commit” wording in `docs/IMPLEMENTATION_COMPLETE.md` so it matches the plural pre-enforcement commits described in `docs/LEGACY_COMMIT_NOTE.md`.
...
- Clarify the shared `AGENTS.md` escalation path: Grok Build remains the preferred secondary implementation agent when available, and an available specialist agent is the fallback when Grok Build is not present in the environment.
...
- Fix `KERNEL_B_BASE_VERSION` detection failure: change `ls -1` to `ls -1d` so that `/usr/src/linux-*` glob lists directory entries themselves instead of their contents, allowing the sed pattern to match `linux-<version>-gentoo` correctly.
...
### Fixed
- Fixed Conventional Commits validation to properly handle legacy commits (grafted and initial commits)
